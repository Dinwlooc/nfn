...
var filtered:Array
var command_suggestions: Array = ["c_start", "c_draw", "c_help", "c_connect_to", "c_close",
								  "c_config", "c_reload", "c_debug", "c_save", "c_load",
								  "c_network", "c_stats", "c_profile", "c_quit", "c_reset",
								  "c_pause", "c_resume", "c_fastforward", "c_screenshot"]
...
func _on_command_submitted(new_text):
	...
	if not command_with_args.is_empty():
		...
		match command:
			"c_start":
				send_signal("c_start",args)
			"c_draw":
				send_signal("c_draw",args)
			"c_help":
				send_signal("c_help",args,1)
			"c_connect_to":
				send_signal("c_connect_to",args,1)
			"c_close":
				send_signal("c_close",args,0)
		Ninput.text = ""  # 清空输入框
 #####主要功能函数#####

func send_signal(signal_name:String,args:Array,arg_num:int = 0):#发送信号和可能的参数组
	var unbind_num = args.size()-arg_num
	if unbind_num>0:
		args.push_front(signal_name)
		get_parent_control().emit_signal.unbind(unbind_num).callv(args)
		GlobalConsole._print(["发送指令：",signal_name,"。参数过滤数量：",unbind_num])
	elif unbind_num == 0:
		args.push_front(signal_name)
		get_parent_control().emit_signal.callv(args)
		GlobalConsole._print(["发送指令：",signal_name])
	else:
		GlobalConsole._print(["Error:参数不足，",signal_name,"未发送。目标：",arg_num])
