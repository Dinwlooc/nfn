...
func tween_animations(node:Node,list:Dictionary,time):
	var tween:Tween = node.create_tween()
	if list:
		for key in list:
			tween.tween_property(node,key,list[key],time).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)

func generate_coordinates(card_container_positoin:Vector2,card_container_size:Vector2,card_count:int = 1) -> Array:
	if card_count == 0:
		return []
	# 获取区域尺寸
	...
	# 处理单张卡牌特殊情况
	if card_count == 1:
		return [Vector2(width / 2, y_center)]
	# 计算等间距分布
	var coordinates = []
	for i in range(card_count):
		# 使用线性插值计算位置 (0到width的等比位置)
		var x = ((i as float + 1) / (card_count + 1)) * width
		coordinates.append(Vector2(x, y_center)+card_container_positoin)
	return coordinates 
