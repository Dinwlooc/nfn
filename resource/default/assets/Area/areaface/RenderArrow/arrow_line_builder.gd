...
func build_enemy_line(
	...
	return true
## 从防御区牌池中选取非本地的敌方牌（优先取最上层非本地牌）
func _find_enemy_card(pool: Array[RenderItem], local_id: int) -> RenderItem:
	...
	if pool.size() < 2:
		return null
	var second_card: RenderItem = pool[-2]
