...
func card_move(
	...
	var preview_params: Dictionary = _compute_preview_params(cards, local_player_id, viewport) if preview_mode and pool_size >= 1 else {}
	for i in pool_size:
...
func _animate_single_card(
	...
	var is_preview_second: bool = preview_mode and (index == pool_size - 2) and preview_params.get("has_second", false)
