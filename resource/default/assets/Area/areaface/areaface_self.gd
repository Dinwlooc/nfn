...
func _handle_damage_event(event: RenderEvent) -> void:
	...
	if hp_damage > 0:
		...
		if _current_tween == null:
			...
			if character is Sprite2D:
				character.frame = 1  # 假设 0 为正常帧，1 为受击帧
	...
	var flash_duration: float = TOTAL_TIME_FACTOR * MIN_DURATION + (MAX_DURATION - MIN_DURATION) * clampf(hp_damage / DAMAGE_REFERENCE, 0.0, 1.0)
	if hp_damage > 0 and character and character.material is ShaderMaterial:
