...
func quickly_select_the_target():
	...
	if !areatargets:
		return
	if get_selected_cards().size() == 1 && get_selected_cards()[0].data.name == RenderCard.DefaultType.ATTACK && areatargets.card_pool.size() > 0 && areatargets.get_selected_cards().size() == 0:
