...
func _on_init_phase(game_state: GameState, context_overriding: CardMoveCommand.Context = _context as Context) -> void:
	...
	if not (
		...
		RuleGuard.has_valid_area(drawing_area, _guard_error, "无法获取牌堆区域") and
		RuleGuard.has_valid_area(game_state.get_hand_area(source_player.get_id()), _guard_error, "无法获取玩家手牌区域")
	):
